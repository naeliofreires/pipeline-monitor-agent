# Enrich anomalies with flow health, run history, and error logs before classifying

Before the agent asks the LLM to classify and explain an Anomaly, it first gathers the real state of the flow: its current health status, the status of the latest run, the record and error counts for that run, recent run-summary trends, and the actual ERROR log lines from the latest/recent runs it can inspect. This enrichment happens in code, after detection and before classification. The LLM then reasons over this Evidence instead of over the notification message alone.
...
- `flows.search_flow_logs(flow_id, severity="ERROR", run_ids=...)` — the actual ERROR log lines for the latest/recent runs being checked
- `metrics.get_resource_metrics_by_run(...)` — per-run `records` / `errors` / `size`
- `/data_sources/{source_id}/metrics/run_summary` and `/data_sinks/{destination_id}/metrics/run_summary` — recent run summaries used to derive compact historical Evidence such as recent run count, average previous records, latest records from summary, record-drop percentage, latest errors from summary, and consecutive failed runs. The resource attribution is the endpoint/resource used for the call, not a field inside each returned run row.
...
The LLM prompt must also make the Alert explanation explicit about the log check: it says the agent checked logs from the latest/recent runs of the Flow being analyzed, summarizes any Nexla ERROR log Anomalies found, says when none were found, or marks the check inconclusive when Evidence is partial.

**Consequence**: each anomaly now triggers a small number of extra read-only Nexla API calls (health, run status, run metrics, optional run summary, and ERROR logs) before the single LLM call. This costs latency and adds rate-limit pressure per anomaly — acceptable because the LLM is still only called once per real problem, not per cycle. If any enrichment call fails, the agent degrades gracefully to the message-only payload and still produces an Alert; an Anomaly is never dropped because enrichment failed. Enrichment also depends on resolving a notification's `resource_id`/`resource_type` to the owning flow, because the health/logs/run-status endpoints key off the flow's id, not the data source/set/sink that raised the notification.
