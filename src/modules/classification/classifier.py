...
def _payload(anomaly: Anomaly, evidence: Evidence) -> dict[str, Any]:
    return {
        ...
        "owner_email": anomaly.owner_email,
        "org_name": anomaly.org_name,
        "access_roles": list(anomaly.access_roles),
        "read_at": str(anomaly.read_at) if anomaly.read_at is not None else None,
        "created_at": str(anomaly.created_at) if anomaly.created_at is not None else None,
        "updated_at": str(anomaly.updated_at) if anomaly.updated_at is not None else None,
